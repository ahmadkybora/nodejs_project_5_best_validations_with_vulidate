<template>
    <div class="container">
        <h3 class="text-center">Popular ArticleCategories</h3>
        <!--//-->
        ...
        <div class="row my-5">
            <carousel :autoplay="true" :nav="false">
                <div v-for="category in popularCategories" :key="category.id" class="col-md-2">
...
<script>
    ...
    export default {
        name: "PopularArticlesCategories",
        components: {carousel},
        data() {
            return {
                loading: true,
                item: ArticleCategoryItems,
                search: '',
            }
        },
        mounted() {
            this.loading = false;
            return this.$store.dispatch('ArticleCategories/popularArticleCategories');
        },
        computed: mapState({
            categories: state => state.Articles.popularArticleCategories
        }),
        methods: {
            likes(product) {
                let likes = {
